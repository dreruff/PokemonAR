...
namespace SQLiter
{
	...
	public class SQLite : MonoBehaviour
	{
		...
		/// <returns></returns>
		public ArrayList GetPokemon (string id)
		{
			return QueryById (id);
		}
		...
		public ArrayList QueryById (string id)
		{
			...
			try {
				_connection.Open ();
				_command.CommandText = "SELECT * FROM " + SQL_TABLE_NAME + " WHERE " + COL_ID + "='" + id + "'";
				_reader = _command.ExecuteReader ();

				_reader.Read ();

				result.Add (_reader.GetString (0));
				result.Add (_reader.GetString (1));
				result.Add (_reader.GetString (2));
				result.Add (_reader.GetString (3));
